...
```{r}
time_plot_3m <- ggplot(df, aes(x = date, y = y_lr_3m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "Time Plot of Log Oil Price Returns (3m)",
...
```{r}
target_ts <- ts(df$y_lr_3m, frequency = 12)
...
This analysis searches for leading indicators (-ve lags) by calculating the cross-correlation between the current value of each predictor and the future values of the target variable, y_lr_3m.
```{r}
df2 <- df %>%
  select(-date, -y_lr_3m) %>% select(ends_with("_l0")) %>%
  names()

...
```{r}
time_plot_1m <- ggplot(df, aes(x = date, y = y_lr_1m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "Time Plot of Log Oil Price Returns (1m)",
...
The time plot of y_lr_1m, the target variable shows several crucial characteristics. First, the series is mean-reverting and stationary. Secondly, it displays distinct periods of volatility clustering and significant structural breaks corresponding to major economic events (eg 2020). These characteristics violate the homoscedasticity assumption of linear models, and hence favor machine learning methods that can handle non-constant variance and high-magnitude outliers. Thirdly, the presence of these breaks mandates the use of rolling-window or walk-forward validation over a fixed train-test split to ensure that the final model's reliability is accurately assessed under changing market trends.

## 2. Density Plot Analysis
```{r}
# Density Plot
density_plot_1m <- ggplot(df, aes(x = y_lr_1m)) +
  geom_density(color = "red", linewidth = 1) +
  labs(
    title = "Distribution of Log Oil Price Returns (1m)",
    x = "Log Return", y = "Density") +
  theme_minimal()
print(density_plot_1m)
```

The density plot of the 1-month log returns shows the data is not normally distributed, where there are long tails on both the left and right sides of the plot, though not as extreme as the 12-month horizon. This is especially crucial for LASSO and Ridge regression models that rely on the assumption of normally distributed residuals. The plot is also somewhat more symmetrical, but has more weight on the left tail, suggesting downside risk might be marginally heavier than upside risk. This shows that extreme market movements—big crashes happen frequently and cannot be predicted by a normal distribution. Therefore, the model evaluation should not rely solely on standard error metrics like RMSE but should also include risk-adjusted performance metrics that penalise downside risk more heavily.
...
```{r}
target_ts <- ts(df$y_lr_1m, frequency = 12)
...
par(mfrow = c(1, 1)) 
```
The ACF measures the total correlation between the current oil return and its past values. There is a statistically significant positive correlation at lag 1. After that, the correlations quickly drop into the insignificant range, though smaller spikes appear in later lags.
...
This analysis searches for leading indicators (-ve lags) by calculating the cross-correlation between the current value of each predictor and the future values of the target variable, y_lr_1m.
```{r}
df2 <- df %>%
  select(-date, -y_lr_1m) %>% select(ends_with("_l0")) %>%
  names()
